...
export default function TourDetails() {
  ...
  return (
      <div className="container mx-auto py-12 px-4 dark:bg-slate-900 transition-colors">
        <Link to="/tours" className="text-slate-500 hover:text-primary mb-6 inline-flex items-center gap-2 transition-colors">
          <span>←</span> До каталогу
        </Link>

        <div className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-xl border dark:border-slate-700">
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[120] flex items-center justify-center p-4">
              <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative border dark:border-slate-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="pt-4 flex flex-col gap-3">
                    <button type="submit" className="w-full bg-primary py-5 rounded-2xl text-white font-black text-lg shadow-xl hover:bg-sky-600 transition-all active:scale-95">
                      ПІДТВЕРДИТИ
                    </button>
                    <button type="button" onClick={() => setIsModalOpen(false)} className="w-full text-slate-400 font-bold hover:text-red-500 transition-colors py-2">
                      Скасувати
                    </button>
                  </div>
                </form>
