  ...
  return (
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] bg-slate-50 dark:bg-slate-900 transition-colors duration-500">
        ...
        <div className="w-full lg:w-64 bg-slate-800 text-slate-300 flex flex-col border-b lg:border-r border-slate-700 shadow-xl shrink-0">
          <div className="p-4 lg:p-6 font-bold text-xl text-white border-b border-slate-700 flex items-center justify-between lg:justify-start gap-3 text-center">
          ...
          <nav className="flex lg:flex-col flex-wrap overflow-x-auto lg:overflow-y-auto px-2 lg:px-4 py-3 lg:py-6 gap-2 no-scrollbar">
            {[
        ...
        <div className="flex-1 p-4 lg:p-8 overflow-y-auto overflow-x-hidden">
          ...
          {activeTab === 'overview' && (
              <div className="animate-in fade-in duration-500">
                <h1 className="text-2xl lg:text-3xl font-black mb-8 text-slate-800 dark:text-white uppercase tracking-tighter">Статистика</h1>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Відгуки', val: reviews.length, color: 'text-yellow-500' }
                      ...
                      <div key={i} className="bg-white dark:bg-slate-800 p-5 rounded-2xl lg:rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm transition-transform hover:scale-[1.02]">
          ...
          {/* 2. ЗАЯВКИ (Адаптивні: Таблиця -> Картки з редагуванням) */}
          {activeTab === 'bookings' && (
              <div className="animate-in slide-in-from-bottom-4 duration-500">
                <h1 className="text-2xl lg:text-3xl font-black mb-8 text-slate-800 dark:text-white uppercase tracking-tighter">Керування заявками</h1>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl lg:rounded-3xl border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm">
                  ...
                  <div className="hidden md:block">
                    <table className="w-full text-left">
                      ...
                      <tr>
                        ...
                        <th className="p-4 text-right text-[10px] uppercase font-black text-slate-400">Статус</th>
                      </tr>
                      </thead>
                      <tbody>
                      {bookings.map(booking => (
                          <tr key={booking.id} className={`border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${booking.status === 'Нова' ? 'bg-primary/5' : ''}`}>
                            {editingId === booking.id ? (
                                <>
                                  ...
                                  <td className="p-4 text-right flex flex-col items-end gap-2">
                                    <button onClick={saveEdit} className="bg-green-500 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md">ОК</button>
                                    <button onClick={() => setEditingId(null)} className="text-slate-400 text-[10px] hover:text-red-500">Скасувати</button>
                                  </td>
                                </>
                            ) : (
                                <>
                                  <td className="p-4">
                                    ...
                                    <div className="text-xs text-slate-500 dark:text-slate-400">{booking.phone}</div>
                                  </td>
                                  <td className="p-4">
                                    <div className="text-primary dark:text-sky-400 font-bold text-sm">{booking.tourName}</div>
                                    <div className="text-[10px] text-slate-400">{booking.date}</div>
                                  </td>
                                  <td className="p-4 text-right">
                                    <div className="flex justify-end items-center gap-3">
                                      <button onClick={() => startEditing(booking)} className="p-2 text-slate-400 hover:text-primary transition-colors">✏️</button>
                                      <button onClick={() => {if(window.confirm('Видалити?')) deleteBooking(booking.id)}} className="p-2 text-slate-400 hover:text-red-500 transition-colors">🗑️</button>
                                      <select
                                          value={booking.status}
                                          onChange={(e) => updateBookingStatus(booking.id, e.target.value as BookingStatus)}
                                          className="text-[9px] font-black p-2 rounded-lg border dark:bg-slate-700 dark:text-white dark:border-slate-600 outline-none"
                                      >
                  ...
                  {/* CARDS FOR MOBILE (FIXED REDACTING) */}
                  <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
                    {bookings.map(booking => (
                        <div key={booking.id} className={`p-4 space-y-3 ${booking.status === 'Нова' ? 'bg-primary/5' : ''}`}>
                          {editingId === booking.id ? (
                              /* РЕЖИМ РЕДАГУВАННЯ НА МОБІЛЦІ */
                              <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                                <div className="space-y-2">
                                  <input className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-700 border dark:border-slate-600 rounded-xl dark:text-white" value={editForm.clientName} onChange={e => setEditForm({...editForm, clientName: e.target.value})} placeholder="Ім'я" />
                                  <input className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-700 border dark:border-slate-600 rounded-xl dark:text-white" value={editForm.phone} onChange={e => setEditForm({...editForm, phone: e.target.value})} placeholder="Телефон" />
                                </div>
                                <input className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-700 border dark:border-slate-600 rounded-xl dark:text-white" value={editForm.tourName} onChange={e => setEditForm({...editForm, tourName: e.target.value})} placeholder="Назва туру" />
                                <div className="flex gap-2">
                                  <button onClick={saveEdit} className="flex-1 bg-green-500 text-white p-3 rounded-xl font-bold shadow-lg">Зберегти</button>
                                  <button onClick={() => setEditingId(null)} className="flex-1 bg-slate-200 dark:bg-slate-600 dark:text-white p-3 rounded-xl font-bold">Скасувати</button>
                                </div>
                              </div>
                          ) : (
                              /* РЕЖИМ ПЕРЕГЛЯДУ НА МОБІЛЦІ */
                              <>
                                <div className="flex justify-between items-start">
                                  ...
                                  <div className="flex gap-2">
                                    <button onClick={() => startEditing(booking)} className="p-3 bg-slate-100 dark:bg-slate-700 rounded-xl text-lg">✏️</button>
                                    <button onClick={() => {if(window.confirm('Видалити?')) deleteBooking(booking.id)}} className="p-3 bg-slate-100 dark:bg-slate-700 rounded-xl text-lg">🗑️</button>
                                  </div>
                                </div>
                                <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl">
                                  ...
                                  <select
                                      value={booking.status}
                                      onChange={(e) => updateBookingStatus(booking.id, e.target.value as BookingStatus)}
                                      className="text-[10px] font-black p-2 rounded-xl bg-white dark:bg-slate-600 border-none shadow-sm outline-none"
                                  >
                                    <option value="Нова">НОВА</option>
                                    <option value="Оплачено">ОПЛАТА</option>
                                    <option value="Відмінено">ВІДМІНА</option>
          ...
          {activeTab === 'tours' && (
              <div className="animate-in slide-in-from-bottom-4 duration-500">
                <h1 className="text-2xl lg:text-3xl font-black mb-8 text-slate-800 dark:text-white uppercase tracking-tighter">Керування Турами</h1>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 mb-8 shadow-sm">
                  <form onSubmit={handleAddTour} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" placeholder="Назва" required value={newTour.name} onChange={e => setNewTour({...newTour, name: e.target.value})} className="p-3 bg-slate-50 dark:bg-slate-700 dark:text-white border border-slate-100 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-primary text-sm" />
                    <input type="number" placeholder="Ціна ($)" required value={newTour.price} onChange={e => setNewTour({...newTour, price: e.target.value})} className="p-3 bg-slate-50 dark:bg-slate-700 dark:text-white border border-slate-100 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-primary text-sm" />
                    <textarea placeholder="Опис..." required value={newTour.desc} onChange={e => setNewTour({...newTour, desc: e.target.value})} className="p-3 bg-slate-50 dark:bg-slate-700 dark:text-white border border-slate-100 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-primary md:col-span-2 h-20 resize-none text-sm" />
                    <div className="md:col-span-2 flex justify-end">
                      <button type="submit" className="bg-green-500 text-white font-bold py-2 px-8 rounded-xl shadow-lg hover:scale-105 transition-transform text-sm">+ ДОДАТИ</button>
                    </div>
                  </form>
                </div>
          ...
          {activeTab === 'reviews' && (
              <div className="animate-in slide-in-from-bottom-4 duration-500">
                <h1 className="text-2xl lg:text-3xl font-black mb-8 text-slate-800 dark:text-white uppercase tracking-tighter">Відгуки</h1>
                  ...
                  {reviews.map(review => (
                      <div key={review.id} className="bg-white dark:bg-slate-800 p-5 rounded-2xl lg:rounded-3xl border border-slate-100 dark:border-slate-700 flex flex-col md:flex-row justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 italic">"{review.text}"</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <select
                              value={review.status}
                              onChange={(e) => updateReviewStatus(review.id, e.target.value as ReviewStatus)}
                              className="text-[10px] font-black p-2 rounded-lg border dark:bg-slate-700 dark:text-white"
                          >
                            <option value="Новий">🟡 НОВИЙ</option>
                            <option value="Схвалено">🟢 СХВАЛЕНО</option>
                            <option value="Відхилено">🔴 ВІДХИЛЕНО</option>
                          </select>
                          <button onClick={() => deleteReview(review.id)} className="text-red-500 text-xs font-bold hover:underline">Видалити</button>
                        </div>
                      </div>
                  ))}
                </div>
